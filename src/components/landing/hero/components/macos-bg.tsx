...
// Lazy load p5 for better initial load performance
...
export default function MacOSWaves() {
  ...
  useEffect(() => {
    ...
    // Lazy load p5 after component mounts
    const initP5 = async () => {
      ...
      const Sketch = (s: any) => {
        ...
        s.setup = () => {
          ...
          s.noStroke()
          s.noiseSeed(20)
          s.frameRate(30) // Limit to 30fps for better performance
          setIsLoaded(true)
        }
    ...
    isInitialized = true
    // Delay p5 initialization slightly to prioritize other content
  ...
  return (
    <>
      {/* Show a simple gradient background while p5 loads */}
      {!isLoaded && (
        <div className='absolute inset-0 -z-10 h-full w-full animate-pulse overflow-hidden bg-gradient-to-br from-background via-background to-muted/20' />
      )}
      <div
        ref={sketchRef}
        className='absolute inset-0 -z-10 h-full w-full overflow-hidden transition-opacity duration-500'

...
export const ReelProgress = ({
  ...
  if (typeof children === 'function') {
    return (
      <div
        className={cn(
          'absolute left-0 right-0 top-0 z-40 flex gap-1 p-2',
          className
        )}
        {...props}
      >
    ...
    >
      {data.map((item, index) => (
        <input
          ...
          type='range'
          defaultValue={calculateProgress(index)}
          min='0'
          max='100'
          onChange={() => {}}
...
export const ReelControls = ({ className, ...props }: ReelControlsProps) => (
  <div
    className={cn(
      'absolute bottom-0 left-0 right-0 z-20 flex items-center justify-between p-4',
...
export const ReelHeader = ({ className, ...props }: ReelHeaderProps) => (
  <div
    className={cn(
      'absolute left-0 right-0 top-0 z-20 p-4 pt-6',

...
The team needed a single codebase that could serve a marketing site, a public job board, and an internal dashboard—without sacrificing multilingual support. Traditional monolithic CMSes made localization, authenticated uploads, and real-time admin tools difficult to orchestrate. By adopting Next.js 14’s App Router and a modern React tooling stack, we were able to ship a bespoke experience for candidates and operators alike.
...
- Integrated uploads for imagery and attachments with UploadThing’s dropzone flow.
...
Authentication rides on NextAuth’s credentials provider. When an admin signs in, we look up their record with Prisma, hash new passwords on the fly, and propagate the user ID into the JWT session payload. Prisma itself is wrapped in a singleton so local development avoids spawning duplicate clients.
